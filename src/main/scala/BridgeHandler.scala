...
final class BridgeHandler(
  ...
  private[this] val observerKey =
    AttributeKey.valueOf[Observer[HttpObject]](getClass, "observer")
  private[this] val writableKey =
    AttributeKey.valueOf[Promise[Ack]](getClass, "writable")
  ...
  override def channelWritabilityChanged(
    ...
    if (context.channel.isWritable) {
      for {
        promise <- Option(context.channel.attr(writableKey).getAndSet(null))
      } {
  ...
  override def channelRead0(
    ...
    message match {
      case request: HttpRequest =>
        ...
        for {
          old <- Option(context.channel.attr(observerKey).setIfAbsent(subject))
        } {
          if (old != subject) {
        ...
        handler(request)(subject).runOnComplete {
          case Success((response, body)) =>
            ...
            body.subscribe(
              BridgeObserver[Content](context, observerKey, writableKey)
            )

          case Failure(error) =>
            // TODO: we should return a 500
            logger.error(
              "Got an unhandle exception from the reactive handler",
              error
            )
            context.close()
        }
